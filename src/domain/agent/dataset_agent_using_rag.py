...
class RagVectorStore:
    def __init__(self):
        ...
        self.chroma_store_directory = "chroma_store"
        self.vectorstore = Chroma(
            embedding_function=self._embed,
            persist_directory=self.chroma_store_directory
        )
...
class DatasetAgent:
    def __init__(self):
        """初始化智能体。"""
        self.rag_vector_store = RagVectorStore()
    ...
    async def __call__(self, global_state: AdapterState) -> Command:
        ...
        try:
            ...
            # self.rag_vector_store.add_texts(str(raw_file_tree).splitlines())
            ...
            metadata_enriched_tree = await self._run_stage_3_read_metadata(raw_file_tree, processed_dir, config,
                                                                           read_file_metadata)
            self.rag_vector_store.add_texts(str(metadata_enriched_tree).splitlines())
            ...
            # self.rag_vector_store.add_texts(str(enhanced_tree_dict).splitlines())
    ...
    async def search_files_using_prompt(self, llm: BaseLanguageModel, query_prompt: str, top_k: int = 3) -> List[str]:
        """
        查询工具，适用于上级智能体或用户需要根据提示词搜索文件的场景。
        """
        prompt_for_query = f"你是一个文件搜索专家，下面给你一个包含完整信息的数据集文件树和一个查询需求，请返回与查询最相关的文件名称和路径。只需要返回字符串，不要输出任何无关的内容。\n\n"
        prompt_for_query += f"数据集文件树:\n{json.dumps(self.rag_vector_store.vectorstore.as_retriever().get_relevant_documents(''), ensure_ascii=False, indent=2)}\n\n"
        prompt_for_query += f"查询需求: {query_prompt}\n\n"
        logging.info(f"执行查询: {prompt_for_query}")
        response_message = await llm.ainvoke(prompt_for_query)
        logging.info(f"查询结果: {response_message.content}")
        # 使用 RAG Vector Store 进行查询
        results = self.rag_vector_store.query(response_message.content, top_k=top_k)
        logging.info(f"查询返回 {len(results)} 个结果。")
        return results
